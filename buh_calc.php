  ...
  for($j=0; $j<mysql_num_rows($resj); $j++) {
    ...
    $resi=mysql_query('
     ...
     FROM
      op
     WHERE
      status = 1 and user_id = '.$uid.' and acc_id = '.$temp_acc.' and op_vid = 1 and cat_id <> 110
     GROUP BY op_date
    ');
	//echo "<script>alert('num_rows=".mysql_num_rows($resi)."');</script>";
	//echo "<script>alert('account=".$temp_acc."');</script>";
	$num_ins = 0;
    for($i=0; $i<mysql_num_rows($resi); $i++) {
      ...
      if ($cnt<1) {
	    $num_ins = $num_ins + 1;
		$temp_norm = U2str(mysql_result($resi, $i, 'op_date'));
    ...
    //echo "<script>alert('Обороты дебет');</script>";
	//echo "<script>alert('num_ins=".$num_ins."');</script>";
    ...
    for($k=0; $k<mysql_num_rows($resk); $k++) {
      $cnt=quick_select('
        ...
        ');
	  $temp_norm = U2str(mysql_result($resk, $k, 'op_date'));
      if ($cnt<1) {
        mysql_query('INSERT INTO balance
        (
        status,

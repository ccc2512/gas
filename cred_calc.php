...
<?php
 if ($act='recal') { // ------------------------------- Full Recalculation -------------------------------
  ...
  //Разовая корректировка date in op
    /* 
  $resq=mysql_query('
   SELECT
    id, op_date, norm_date
   FROM
    op
  ');
  for($i=0; $i<mysql_num_rows($resq); $i++) {
    $temp_norm = U2str(mysql_result($resq, $i, 'op_date'));
	//$temp_norm = "31/12/2012";
	mysql_query('
      UPDATE op
      SET norm_date  = "'.$temp_norm.'"
	  WHERE id = '.mysql_result($resq, $i, 'id').'
    ');
  ...
  $resi=mysql_query('
   ...
   (cat_id = 111 or cat_id = 112 or cat_id = 113 or cat_id = 114)
